...
use Xqddd\Notifications\Critical;
...
class ConstructTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testWhenCalledWithProperAttributesWillStoreTheAttributes()
    {
        ...
        $Critical = new Critical($label, $message);

        static::assertSame(
            $label,
            \PHPUnit_Framework_Assert::readAttribute($Critical, 'label')
        );
        static::assertSame(
            $message,
            \PHPUnit_Framework_Assert::readAttribute($Critical, 'message')
        );
    }
    ...
    public function testWhenCalledWithImproperParametersWillThrowAnException($labelValue, $messageValue, $typeValue)
    {
        ...
        try {
            $Critical = new Critical($labelValue, $message);
        } catch (\PHPUnit_Framework_Error $e) {
            static::assertStringStartsWith(
                'Argument 1 passed to Xqddd\Notifications\Critical::__construct() must be an instance of Xqddd\Notifications\Label,',
                $e->getMessage()
            );
        ...
        try {
            $Critical = new Critical($label, $messageValue);
        } catch (\PHPUnit_Framework_Error $e) {
            static::assertStringStartsWith(
                'Argument 2 passed to Xqddd\Notifications\Critical::__construct() must be an instance of Xqddd\Notifications\Message,',
                $e->getMessage()
            );
            static::assertEquals(4096, $e->getCode());
        }
