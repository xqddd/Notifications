...
class ConstructTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testWhenCalledWithProperAttributesWillStoreTheAttributes()
    {
        ...
        $Debug = new Debug($label, $message);

        static::assertSame(
            $label,
            \PHPUnit_Framework_Assert::readAttribute($Debug, 'label')
        );
        static::assertSame(
            $message,
            \PHPUnit_Framework_Assert::readAttribute($Debug, 'message')
        );
    }
    ...
    public function testWhenCalledWithImproperParametersWillThrowAnException($labelValue, $messageValue, $typeValue)
    {
        ...
        try {
            $Debug = new Debug($labelValue, $message);
        } catch (\PHPUnit_Framework_Error $e) {
            static::assertStringStartsWith(
                'Argument 1 passed to Xqddd\Notifications\Debug::__construct() must be an instance of Xqddd\Notifications\Label,',
                $e->getMessage()
            );
        ...
        try {
            $Debug = new Debug($label, $messageValue);
        } catch (\PHPUnit_Framework_Error $e) {
            static::assertStringStartsWith(
                'Argument 2 passed to Xqddd\Notifications\Debug::__construct() must be an instance of Xqddd\Notifications\Message,',
                $e->getMessage()
            );
            static::assertEquals(4096, $e->getCode());
        }
